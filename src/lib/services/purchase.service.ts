...
export const purchaseService = {
  ...
  async getAll(orgId: string): Promise<Purchase[]> {
    ...
    const { data, error } = await supabase
      ...
      .select('*, supplier:corivacore_suppliers(name, phone)')
      .eq('org_id', orgId)
  ...
  async create(orgId: string, purchase: {
    supplier_id?: string
    items: PurchaseItem[]
    notes?: string
    expected_at?: string
    ...
    const { data: numData } = await supabase
      .rpc('generate_purchase_number', { p_org_id: orgId })
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        purchase_number: numData ?? `OC-${Date.now()}`,
        status: 'pending',
        total,
        notes: purchase.notes,
        expected_at: purchase.expected_at ?? null,
    ...
    const { error: itemsError } = await supabase
      .from('corivacore_purchase_items')
      .insert(items)
