...
export async function GET(
  ...
  try {
    ...
    // 1. Intentar caché
    const cached = await documentCache.get('RUC', params.numero)
    if (cached) {
      await documentCache.logQuery('RUC', params.numero, true)
    ...
    // 3. Guardar en caché y auditar
    await Promise.allSettled([
      documentCache.set('RUC', params.numero, result),
      documentCache.logQuery('RUC', params.numero, false),
    ])
