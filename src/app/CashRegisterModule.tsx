...
export default function CashRegisterModule({ currentUser }: CashRegisterProps) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!hasOpenedToday ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-4">📊 Movimientos de Hoy</h3>
            <div className="space-y-3">
              {todayMovements.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No hay movimientos registrados</p>
              ) : (
                todayMovements.map((movement) => (
                  <div key={movement.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <div>
      ...
      {/* Modal Cierre */}
      {/* Funcionalidad de cierre deshabilitada temporalmente */}
    </div>
  )
}
